...
export default function NurturePage() {
  ...
  const fetch_ = useCallback(async () => {
    ...
    setDeals((data.grouped?.connected ?? []) as Deal[]);
  }, []);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const payload = { ...form, stage: "connected", mrr: form.mrr !== "" ? Number(form.mrr) : undefined, contactEmail: form.contactEmail || undefined, closeDate: form.closeDate || undefined };
  ...
  return (
    <PageShell
      title="Nurture"
      description="Accounts with an active response that still need follow-up and progression"
      actions={
        <div className="flex items-center gap-2">
          {selected.size > 0 && (
            <>
              <span className="text-xs text-muted-foreground">{selected.size} selected</span>
              <Button size="sm" disabled={moving} onClick={() => moveSelected("technical_evaluation")}>→ Technical Evaluation</Button>
            </>
          )}
          <Button size="sm" onClick={openCreate}><Plus className="size-3.5 mr-1" /> Add nurture account</Button>
        </div>
      }
    >
      <div className="p-4">
        {deals.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-16 text-center">
            <p className="text-sm font-medium">No nurture accounts yet</p>
            <p className="mt-1 text-xs text-muted-foreground">Move connected accounts here for follow-up and relationship building</p>
            <Button size="sm" className="mt-4" onClick={openCreate}><Plus className="size-3.5 mr-1" /> Add nurture account</Button>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {deal.mrr != null && <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><DollarSign className="size-3 shrink-0" />{deal.mrr.toLocaleString()}/mo</p>}
                    ...
                    <div className="mt-3 flex items-center justify-between border-t border-border pt-2">
                      ...
                      <button className="flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[10px] text-muted-foreground hover:bg-muted hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => { e.stopPropagation(); moveSelected("technical_evaluation"); }}>
                        Tech Eval <ChevronRight className="size-2.5" />
                      </button>
                    </div>
                  </div>
                </div>
      ...
      <Dialog open={dialog.open} onOpenChange={(o) => !o && setDialog({ open: false, deal: null })}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{dialog.deal ? "Edit nurture account" : "Add nurture account"}</DialogTitle></DialogHeader>
          <div className="grid gap-3 py-2">
            ...
            <div><Label>Notes</Label><Textarea rows={3} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Follow-up plan, objections, next touchpoint..." /></div>
          </div>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setDialog({ open: false, deal: null })}>Cancel</Button>
            <Button onClick={save} disabled={saving}>{saving && <Loader2 className="mr-1.5 size-3.5 animate-spin" />}{dialog.deal ? "Save" : "Add nurture account"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </PageShell>
